...
namespace poker
{
    class WinningPotComparer:IComparer
    {

        public int Compare(object a, object b)
        {
            if (((WinningPot)a).Amount_pot == ((WinningPot)b).Amount_pot) return 0;
            if (((WinningPot)a).Amount_pot > ((WinningPot)b).Amount_pot) return 1;
            return -1;
