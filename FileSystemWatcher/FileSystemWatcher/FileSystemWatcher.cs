...
namespace OSX.IO.FileSystemWatcher
{
    class FileSystemWatcher : System.ComponentModel.Component
    {
        ...
        private void CompareStaticFSData(StaticFSData newData, StaticFSData oldData)
        {
            ...
            if (RaiseAllEvents == true)
            {
                for (int n = 0; n < raiseEventCount; n++)
                {
                    ...
                    if (OnChanged != null) OnChanged(new FileSystemEventArgs(WatcherChangeTypes.Changed, newData.ParentDirectory, newData.Name));
            ...
            else
            {
                ...
                {
                    if (OnChanged != null) OnChanged(new FileSystemEventArgs(WatcherChangeTypes.Changed, newData.FullName, newData.Name));
                }
            }
        }
        ...
        private void CompareStaticDirectoryData(StaticDirectoryData newData, StaticDirectoryData oldData)
        {
             ...
             *      If the match is not found, then raise OnCreated
             *      Once done, check old list.  If any are left, then raise OnDeleted for those items.
             *      
            ...
            for (int n = 0; n < newData.Files.Count; n++)
            {
                ...
                else
                {
                    ...
                    if (OnCreated != null) OnCreated(new FileSystemEventArgs(WatcherChangeTypes.Created, newData.FullName, newData.Files[n].Name));
            ...
            for (int n = 0; n < oldData.Files.Count; n++)
            {
                ...
                if (OnDeleted != null) OnDeleted(new FileSystemEventArgs(WatcherChangeTypes.Deleted, oldData.FullName, oldData.Files[n].Name));
            }
            ...
            for (int n = 0; n < newData.Directories.Count; n++)
            {
                ...
                else
                {
                    ...
                    if (OnCreated != null) OnCreated(new FileSystemEventArgs(WatcherChangeTypes.Created, newData.FullName, newData.Directories[n].Name));
                }
        ...
        //Events
        public delegate void OnChangedEventHandler(FileSystemEventArgs e);
        public event OnChangedEventHandler OnChanged;
        public event OnChangedEventHandler OnCreated;
        public event OnChangedEventHandler OnDeleted;
        public delegate void OnRenamedEventHandler(RenamedEventArgs e); public event OnRenamedEventHandler OnRenamed;
